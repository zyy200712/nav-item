  ...
  function insertDefaultCards() {
    db.all('SELECT * FROM menus ORDER BY "order"', (err, menus) => {
      ...
      if (menus && menus.length) {
        ...
        const stmt2 = db.prepare('INSERT INTO cards (menu_id, title, url, logo_url, desc) VALUES (?, ?, ?, ?, ?)');
        let insertCount = 0;
        
        cards.forEach(card => {
          if (menuMap[card.menu]) {
            stmt2.run(menuMap[card.menu], card.title, card.url, card.logo_url, card.desc, function(err) {
              if (err) {
                console.error(`插入卡片失败 [${card.menu}] ${card.title}:`, err);
              } else {
                insertCount++;
                // console.log(`成功插入卡片 [${card.menu}] ${card.title} (总计: ${insertCount})`);
              }
            });
          } else {
        ...
        stmt2.finalize(() => {
          console.log(`所有卡片插入完成，总计: ${insertCount} 张卡片`);
        });
      } else {
        console.log('未找到任何菜单');
      }
